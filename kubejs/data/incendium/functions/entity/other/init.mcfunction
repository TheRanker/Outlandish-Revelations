...
# summoning
execute unless score $disabled in.inferno matches 1 unless score $running in.inferno matches 1 if entity @s[type=item,nbt={Item:{id:"ultimate_star:ultimate_star"}}] if entity @e[type=marker,tag=in.altar,distance=..4] run function incendium:hovering_inferno/summon

execute if score $disabled in.inferno matches 1 if entity @s[type=item,nbt={Item:{id:"ultimate_star:ultimate_star"}}] if entity @e[type=marker,tag=in.altar,distance=..3] run function incendium:hovering_inferno/messages/summon_disabled

execute if score $disabled in.inferno matches 0 if score $running in.inferno matches 1 if entity @s[type=item,nbt={Item:{id:"ultimate_star:ultimate_star"}}] if entity @e[type=marker,tag=in.altar,distance=..3] run function incendium:hovering_inferno/messages/inferno_already_running
